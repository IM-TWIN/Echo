...
void loop() {
  ...
  if (curMillis - prevMillis >= INTERVAL) {
    ...
    */
      ...
      classification = clf.predict(features);  //classification = classify(features) if using model with walking/without walking (generated with chatGPT), new models with frog uses micromlgen lib in python
      ...
      if (classificationIndex >= cbs) {  //before there was CLASSIFICATION_BUFFER_SIZE
        ...
        // Conversion from a numerical label (0,1,2,3) to textual label for Serial Printing, and consequential LED and AUDIO activation  -- WHEN YOU USE MODEL WITHOUT WALKING, EXCHANGE WALKING(2) CASE WITH IGNORING CASE (3). UPDATE: Frog class is the new class (2) (model_all, embeddedtest_wfrog)
    ...
    if (!value.empty()) {
      ...
      if (readValue != lastReadValue) {  ///COMMAND: consecutive equal commands will not work!!!, this helps to have different commands
        ...
        if (readValue == 10){////COMMAND: Start log button will turn on the led strip (orange)
          ...
          bufferIndex=0;
          classificationIndex = 0;  //azzero i buffer, cosi avro la prima classificazione esattamente alla prima accensione del led di classificaione
          classification = 9;
          mostFrequentClass = 9; //analogo numerico di "Null"
    ...
    // Counting the ms passed in the single loop, durata must be < INTERVAL !!!!!!!  
