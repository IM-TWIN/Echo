...
func scale_value(value)->float:
	...
	curr_max = 500 #curr_max if value<curr_max else value
	curr_min = -500 #curr_min if value>curr_min else value
...
func _draw():
	...
	draw_polyline(points_line1, Color.red, 4.0,true)
	draw_polyline(points_line2, Color(0.0, 0.5, 1.0 ), 4.0,true)
	draw_polyline(points_line3, Color(1.0, 1.0, 0.0 ), 3.0,true)
	#for i in range(tickNumber):
