...
func scale_value(value)->float:
	...
	curr_max = 180 #curr_max if value<curr_max else value  (180°)
	curr_min = -180 #curr_min if value>curr_min else value   (-180°)
...
func _draw():
	...
	draw_polyline(points_line1, Color(1.0, 0.0, 1.0), 4.0,true)
	
	
	# valore min
	draw_polyline(points_line2, Color(1, 0, 0.0 ), 4.0,true)
	
	#valore max
	draw_polyline(points_line3, Color(1.0, 1.0, 0.0 ), 4.0,true)
	
	# valore thresh
	#draw_polyline(points_line4, Color(1.0, 1.0, 1.0 ), 4.0,true)
	#for i in range(tickNumber):
